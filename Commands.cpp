...
//            std::cout <<"should print" << (char*)def_prompt.c_str() << std::endl;
//            char* def = (char*)def_prompt.c_str();
//            smash.setCurrPrompt(def);
//            std::cout <<"the curr prompt is " << smash.getCurrPrompt() <<std::endl;
            std::string  def_prompt = "smash> ";
            char* buffer = new char[def_prompt.size()+1];
...
//    std::cout << "we are in add job func" <<std::endl;
    ...
    JobEntry* newJob= new JobEntry(maxJobId+1,job_pid,isStopped
                                   ,cmd_line);
//    std::cout << "job id " << newJob->getJobId() <<std::endl;
    jobsList.insert(jobsList.end(),newJob);
    updateMaxJobId();
//    std::cout << "max job id " << maxJobId <<std::endl;
    removeFinishedJobs();
//    std::cout << "atfer remove - max job id " << maxJobId <<std::endl;
...
{
    auto iterator = jobsList.begin();
    auto tempIt = jobsList.begin();
    while(iterator!=jobsList.end())
    {
        int status =0;
//        std::cout << "the pid we are passing is " << (*iterator)->getJobPid() <<std::endl;
        pid_t wait_res = waitpid( (*iterator)->getJobPid(), &status , WNOHANG);
//        std::cout << "in remove finished jobs" <<std::endl;
//        std::cout << "wait result is " << wait_res <<std::endl;
        if(wait_res!=0)
        {
//            std::cout << "the job we are deleting has id "<<(*iterator)->getJobId()<< std::endl;
            ++tempIt;
            delete (*iterator);
...
void ExternalCommand::execute()
{
    ...
    if(p > 0)//parent
    {
        ...
        {
            smash.getJobsList()->addJob(getCommandLine(),p,false);
        }
        else
        {
            int status;
